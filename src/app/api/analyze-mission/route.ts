...
export async function POST(request: NextRequest) {
  try {
    const { missionName, requirements, proof } = await request.json()

    if (!missionName || !requirements || !proof) {
...
Requisitos da missão: ${requirements}
...
Responda apenas com "SIM" se a prova for suficiente e convincente, ou "NÃO" se não for. Seja rigoroso mas justo.`
    ...
    console.error('Error analyzing mission proof:', error)
    return NextResponse.json(
